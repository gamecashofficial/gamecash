...
#if defined(NDEBUG)
# error "Gamecash cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb824d3851cf53cf19c66d90de6beb2a0c4ebf37b7bef7f666ba06ba0aa617f33");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gamecash: starting difficulty is 1 / 2^12
...
{
    // Gamecash: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Gamecash: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Gamecash: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Gamecash: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gamecash: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gamecash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb824d3851cf53cf19c66d90de6beb2a0c4ebf37b7bef7f666ba06ba0aa617f33");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "25/09/2017 Revoloutionising the Gaming industry";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x24b93d5a7a65740d5f019fe0dd05c54994b3793f4bc06662509ab28dabf52eeb"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Gamecash: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GamecashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GamecashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GamecashMiner(CWallet *pwallet)
{
    printf("GamecashMiner started\n");
    ...
    try { loop {
        ...
        printf("Running GamecashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GamecashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GamecashMiner, pwallet));
}
